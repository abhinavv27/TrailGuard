      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <Card>
              <h3 className="text-sm font-medium text-slate-300 mb-3">Evidence Timeline</h3>
                  ...
                  {caseData.evidence.map((ev: any, idx: number) => (
                    <div key={idx} className="flex gap-3">
                      ...
                      <div className="flex-1 pb-3">
                        <p className="text-sm text-slate-200">{ev.title || ev.type || "Evidence"}</p>
            ...
            <Card>
              <h3 className="text-sm font-medium text-slate-300 mb-3">Analyst Notes</h3>
          ...
          <div className="space-y-6">
            <Card>
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-slate-200">{caseData?.alert_count || 0}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Accounts</span>
                  <span className="text-slate-200">{caseData?.account_count || 0}</span>
                </div>
                <div className="flex justify-between">
              ...
              <Card>
                ...
                <div className="space-y-2">
                  {caseData.linked_accounts.map((acc: any, idx: number) => (
                    <div
                      key={idx}
                      className="flex items-center justify-between bg-navy-700/50 rounded-lg px-3 py-2 cursor-pointer hover:bg-navy-700 transition-colors"
                      onClick={() => router.push(`/accounts/${acc.id}`)}
                    >
                      <span className="text-sm text-slate-200 font-mono text-xs">
                        {acc.id?.slice(0, 16)}...
                      </span>
            ...
            <Card>
              <h3 className="text-sm font-medium text-slate-300 mb-3">AI Summary</h3>
