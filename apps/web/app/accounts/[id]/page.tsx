    ...
    <AppShell>
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-1">
          <h1 className="text-2xl font-bold text-slate-900">Account Intelligence Profile</h1>
          {accountLoading ? (
          ...
          <>
            <Card>
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-sm text-slate-500">Incoming Value</p>
            ...
            <Card>
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-sm text-slate-500">Outgoing Value</p>
            ...
            <Card>
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-sm text-slate-500">Unique Counterparties</p>
                  <p className="text-2xl font-bold text-slate-900 mt-1">{metrics.unique_counterparties || 0}</p>
                </div>
            ...
            <Card>
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-sm text-slate-500">Avg Holding Time</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <Card className="lg:col-span-1">
          <h3 className="text-sm font-medium text-slate-700 mb-3">Graph Metrics</h3>
          <div className="space-y-3">
            {accountLoading ? (
              ...
              <>
                {[
                  { label: "Degree Centrality", value: metrics.degree_centrality, color: "text-blue-600" },
          ...
          {accountLoading ? (
            ...
            <div className="space-y-2">
              {patterns.map((pattern: any, idx: number) => (
                <div key={idx} className="flex items-center justify-between bg-slate-100/50 rounded-lg px-4 py-3">
                  <div className="flex items-center gap-2">
                    <AlertTriangle size={14} className="text-amber-400" />
                    <span className="text-sm text-slate-800">{pattern.name || pattern.type}</span>
                  </div>
      ...
      <Card className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-medium text-slate-700">Linked Transactions</h3>
        ...
        {transactions && transactions.length > 0 ? (
          <Table
            columns={[
              { key: "id", header: "TX ID", render: (t: any) => <span className="font-mono text-xs text-slate-700">{t.id?.slice(0, 16)}...</span> },
              { key: "source", header: "Source", render: (t: any) => <span className="text-xs text-slate-500">{t.source_account?.slice(0, 12)}...</span> },
              { key: "destination", header: "Destination", render: (t: any) => <span className="text-xs text-slate-500">{t.destination_account?.slice(0, 12)}...</span> },
              { key: "amount", header: "Amount", render: (t: any) => <span className="text-sm font-medium text-slate-800">${(t.amount || 0).toLocaleString()}</span> },
      ...
      <Card>
        <h3 className="text-sm font-medium text-slate-700 mb-3">Evidence</h3>
        {account?.evidence?.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {account.evidence.map((ev: any, idx: number) => (
              <div key={idx} className="bg-slate-100/50 rounded-lg p-3 border border-slate-200">
                <div className="flex items-center gap-2 mb-2">
                  <FileText size={14} className="text-blue-600" />
                  <span className="text-xs font-medium text-slate-800">{ev.title || ev.type || "Evidence"}</span>
                </div>
